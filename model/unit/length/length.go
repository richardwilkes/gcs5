...
// FromStringForced creates a new Length. May have any of the known Units suffixes, a feet and inches format (e.g. 6'2"),
// or no notation at all, in which case defaultUnits is used.
func FromStringForced(text string, defaultUnits Units) Length {
	length, err := FromString(text, defaultUnits)
	if err != nil {
		return 0
	}
	return length
