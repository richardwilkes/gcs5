...
func LengthFromInt64(value int64, unit LengthUnits) Length {
	return Length(unit.ToInches(f64.From[fxp.DP](value)))
}
...
func LengthFromString(text string, defaultUnits LengthUnits) (Length, error) {
	...
	if feetIndex != -1 {
		...
		feet, err = f64.FromString[fxp.DP](s)
		if err != nil {
			return 0, err
		}
	}
	if inchIndex != -1 {
		...
		inches, err = f64.FromString[fxp.DP](s)
		if err != nil {
			return 0, err
		}
	}
	return Length(feet.Mul(f64.From[fxp.DP](12)) + inches), nil
}
